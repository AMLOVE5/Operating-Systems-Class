...
int get_running_count()
{
    ...
    int count_13 = 0; // count of medians divisible by 13
    printf("[MemoryManager] Number of Iterations: %d\n", iterations);
    // 2a
    for (int i = 0; i < iterations; i++) {
        int arr_size = get_arr_size(rand()); // map rand()
        // 2b
        int *arr = (int*)malloc(arr_size * sizeof(int)); // allocate memory in HEAP
        // 2c
        for (int j = 0; j < arr_size; j++) { 
            arr[j] = rand(); // popular array with rand() numbers
        }
        //2d
        int median = return_median(arr, arr_size); //calc median
        // 2e
        if (median % 13 == 0) {
            count_13 ++; //divisible by 13?
        }
        free(arr); // free memory off HEAP
    }
    //2f
    return count_13; //return count_13
}
